<x-app-layout>
    ...
    <div
        class="py-4"
        x-data
        x-on:open-driver-create-modal.window="$dispatch('open-modal', 'add-driver-modal')"
    >
    ...
    <x-modal name="add-driver-modal" :show="$errors->any()" max-width="2xl" focusable>
        <form method="POST" action="{{ route('drivers.store') }}" class="p-6">
            ...
            <div class="flex items-start justify-between gap-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-600">Create a driver without leaving this page.</p>
                </div>

                <button
                    type="button"
                    x-on:click="$dispatch('close-modal', 'add-driver-modal')"
                    class="rounded-md p-2 text-gray-400 transition hover:bg-gray-100 hover:text-gray-600"
                    aria-label="Close add driver window"
                >
